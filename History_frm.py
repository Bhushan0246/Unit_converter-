...
def openHistory():
    ...
    img = CTkImage(light_image = Image.open(".\images\history_img.png"), size=(220,525))
    main_img = CTkLabel(root, text='', image=img)
    main_img.place(x=638, y=10)

    title = CTkLabel(root, text=' History ', font=font1, text_color='#78ba80')
    title.place(x=245, y=15)

    back_btn = CTkButton(root, text='Back', width=215, height=30, corner_radius=5, border_width=2, border_color='#95C89B', bg_color=color_bg, fg_color=color_bg, text_color='#95C89B',
                                  font=font2, hover_color=color_bg, state=NORMAL, command=go_back)
    back_btn.place(x=635, y=550)

    heading = " Date-Time\tFrom \t  To\t    Value \t          Result"
    sbfrm = CTkScrollableFrame(root, width=595, height=460, corner_radius=8, border_width=1, bg_color='#C3E4C0', fg_color='#C3E4C0', scrollbar_fg_color='#C3E4C0', scrollbar_button_color='#ddeedf',
